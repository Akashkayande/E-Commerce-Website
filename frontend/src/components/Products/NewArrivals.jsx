...
const NewArrivals = ()=>{
    ...
    const handleMouseDown = (e)=>{
        setIsDragging(true);
        setStartX(e.pagesX - scrollRef.current.offsetLeft);
  ...
  return (
    <section className='py-16 px-4 lg:px-0'>
        <div className='container mx-auto text-center mb-10 relative'>
            ...
            <div className="absolute right-0  md:bottom-[-30px] bottom-[-37px] flex space-x-2">
        ...
        <div ref={scrollRef}  className={`container scrollbar-hide mx-auto overflow-x-scroll flex space-x-6 relative ${isDragging ? "cursor-grabbing":"cursor-grab"}`} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUpOrLeave} onMouseLeave={handleMouseUpOrLeave}>
            {newArrivals?.map((product)=>(
                <div key={product._id} className='min-w-[100%] sm:min-w-[50%] lg:min-w-[30%] relative'>
                    <img  src={product.images[0]?.url} alt={product.images[0]?.altText || product.name} className='w-full h-[700px] object-cover rounded-lg' draggable="false" />
                    <div className='absolute bottom-0 left-0 right-0 bg-opacity-50 backdrop-blur-md font-extrabold p-4 rounded-b-lg'>
